...
describe('cors middleware', () => {
  ...
  describe('Integration', () => {
    it('should handle complete CORS flow: preflight + actual request', async () => {
      ...
      expect(actualRes.headers.get('Access-Control-Allow-Origin')).toBe('http://localhost:5173');
      expect(actualRes.headers.get('Access-Control-Allow-Credentials')).toBe('true');
