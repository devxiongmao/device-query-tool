...
describe("env.ts", () => {
  ...
  describe("Schema validation - Valid configurations", () => {
    ...
    it("should parse environment with custom values overriding defaults", () => {
      ...
      expect(result.GRAPHQL_MAX_DEPTH).toBe(3);
      expect(result.GRAPHQL_MAX_COMPLEXITY).toBe(300);
      expect(result.GRAPHQL_RATE_LIMIT_PER_MINUTE).toBe(40);
      expect(result.GRAPHQL_RATE_LIMIT_PER_HOUR).toBe(2000);
    });
